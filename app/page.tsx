...
import { APPROVED_CONTENT_QUERY } from "@/gql/view-contents-query";
import { useRouter } from "next/navigation";
import { useAppSelector } from "@/store/hooks";
import ViewContentModal from "@/components/ViewModal";
...
export default function Home() {
    ...
    const { data, loading, error } = useQuery<any>(APPROVED_CONTENT_QUERY, {
    ...
    const contents = data?.approvedContent ?? [];
    ...
    return (
        <div className="min-h-screen bg-gray-50 px-6 py-10">
                ...
                {contents.map((item: any) => (
                    ...
                    >
                        ...
                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600">
                            ...
                            {/* <div>
                                {(item.assignedModerator || item.status) ? (
                                    <>
                                        <p>
            ...
            {isOpen && <ViewContentModal contentId={selectedContentId} onClose={() => setIsOpen(false)} />}
        </div>
    );
}
